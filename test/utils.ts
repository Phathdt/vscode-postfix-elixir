...
export const getCurrentDelay = () =>
  delaySteps.length > 1 ? <number>delaySteps.shift() : delaySteps[0];
...
export function testTemplateWithQuickPick(
  dslString: string,
  trimWhitespaces?: boolean,
  skipSuggestions = 0,
  cancelQuickPick = false
) {
...
async function selectAndAcceptSuggestion(
...
function assertText(
  ...
  assert.strictEqual(
    ...
    normalizeWhitespaces(expectedResult)
  );
}
...
export function runWithCustomTemplate(template: string) {
  const postfixConfig = vsc.workspace.getConfiguration("postfixElixir");
...
type ParametersSkipFirst<T extends (...args: unknown[]) => void> = T extends (
  first: TestFunction,
  ...args: infer P
) => void
  ? P
  : never;
type TestFnSkipFirstParam<T extends (...args: unknown[]) => void> = (
  ...args: ParametersSkipFirst<T>
) => void;
type RunTestFn<T extends (...args: unknown[]) => void> =
  TestFnSkipFirstParam<T> & {
    only: TestFnSkipFirstParam<T>;
    skip: TestFnSkipFirstParam<T>;
  };

export function makeTestFunction<
  T extends (first: TestFunction, ...args: unknown[]) => void
>(testFn: T) {
