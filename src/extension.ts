...
export function activate(context: vsc.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vsc.commands.registerTextEditorCommand(
      ...
      async (
        ...
        _: vsc.TextEditorEdit,
        ...args: ts.BinaryExpression[]
      ) => {
  ...
  context.subscriptions.push(
    vsc.workspace.onDidChangeConfiguration((e) => {
      if (!e.affectsConfiguration("postfixElixir")) {
        return;
      }
...
function registerCompletionProvider(context: vsc.ExtensionContext) {
  ...
  const TESTS_SELECTOR: vsc.DocumentSelector = ["postfixElixir", "html"];
  const DOCUMENT_SELECTOR: vsc.DocumentSelector =
    process.env.NODE_ENV === "test"
      ? TESTS_SELECTOR
      : <string[]>(
          vsc.workspace.getConfiguration("postfixElixir").get("languages")
        );

  completionProvider = vsc.languages.registerCompletionItemProvider(
    DOCUMENT_SELECTOR,
    provider,
    "."
  );
