...
import {
  adjustLeadingWhitespace,
  adjustMultilineIndentation,
} from "./utils/multiline-expressions";
...
export class CompletionItemBuilder {
  ...
  private constructor(
    ...
    this.node = node;
    this.item = new vsc.CompletionItem(
      { label: keyword, description: "postfixElixir" },
      vsc.CompletionItemKind.Snippet
    );
    this.code = adjustMultilineIndentation(
      node.getText(),
      indentInfo?.indentSize
    );
  }
  ...
  ) => new CompletionItemBuilder(keyword, node, indentInfo);
  ...
  public replace = (replacement: string): CompletionItemBuilder => {
    ...
    const nodeStart = ts.getLineAndCharacterOfPosition(
      src,
      this.node.getStart(src)
    );
    ...
    if (useSnippets) {
      ...
      this.item.additionalTextEdits = [vsc.TextEdit.delete(rangeToDelete)];
