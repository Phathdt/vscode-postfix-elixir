...
export abstract class BaseTemplate implements IPostfixTemplate {
  ...
  protected isSimpleExpression = (node: ts.Node) =>
    ts.isExpressionStatement(node) && !isStringLiteral(node);
  protected isPropertyAccessExpression = (node: ts.Node) =>
    ts.isPropertyAccessExpression(node);
  protected isElementAccessExpression = (node: ts.Node) =>
    ts.isElementAccessExpression(node);
  protected isExpression = (node: ts.Node) =>
    this.isSimpleExpression(node) ||
    this.isPropertyAccessExpression(node) ||
    this.isElementAccessExpression(node);
  protected isIdentifier = (node: ts.Node) =>
    ts.isIdentifier(node) && !this.inTypeReference(node.parent);

  protected isUnaryExpression = (node: ts.Node) =>
  ...
  protected inFunctionArgument = (node: ts.Node) =>
  ...
  protected isObjectLiteral = (node: ts.Node) => {
    return (
      ...
      (node.statements.length === 0 ||
        node.statements.some((x) => ts.isLabeledStatement(x)))
    );
  };
  ...
  protected inAwaitedExpression = (node: ts.Node) => {
    ...
    return (
      ...
      (node.parent && this.inAwaitedExpression(node.parent))
    );
  };
  ...
  protected inReturnStatement = (node: ts.Node) => {
    ...
    return (
      ...
      (node.parent && this.inReturnStatement(node.parent))
    );
  };
  ...
  protected inVariableDeclaration = (node: ts.Node) => {
    ...
    return (
      ...
      (node.parent && this.inVariableDeclaration(node.parent))
    );
  };
